// create a class for sale items: define a constructor
...
function displayCart()
{
    ...
    else
    {
        ...
        {
            var key = sessionStorage.key(x); // index comes from x to loop through session storage
            output += "<tr><td><img src='images/img"+ getID(key) +".jpg' width='50px' height='50px'></td>"; // img of item
            output += "<td>" + key + "</td><td>" + sessionStorage.getItem(key) + "</td>"; // name and price of item
            output += "<td><input type='button' class='btn btn-primary' value='Delete' onclick='remove(\"" + key + "\")'></td></tr>"; // get delete button and config arg for remove function
